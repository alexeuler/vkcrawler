  ...
  class Api
    describe "Vk::Api" do
      ...
      it "Replaces _ with . and converting hash of arguments to request params when called method missing" do
        ...
        api.should_receive(:send).with("https://api.vk.com/method/users.get?access_token=123&uid=whatever&v=0.5\n#{EOF}\n").and_return("success")
      ...
      it "Can batch requests and send them at once" do
        api=Api.new
        Token.stub_chain(:first, :token).and_return("123")
        api.should_receive(:send).with("https://api.vk.com/method/users.get?access_token=123&uid=whatever&v=0.5\n#{EOF}\nhttps://api.vk.com/method/friends.get?access_token=123\n#{EOF}\n").and_return("success")
